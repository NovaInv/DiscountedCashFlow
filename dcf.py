...
def use_revenue_to_forecast():
	...
	for i in range(1,periodsToPredict+1):
		fcf = (IS_forecast.loc['Operating Income'][f'Year{i}'] - IS_forecast.loc['Income Tax Provision'][f'Year{i}'] + IS_forecast.loc['DA Expense'][f'Year{i}']
...
def use_past_fcf_to_forecast():
	...
	ulFCF = (IS['Operating Income'] - IS['Income Tax Provision'] + IS['DA Expense'] - BS['NWC'] - BS['NCS']).values # unlevered free cash flow (ulfcf)
