...
def use_revenue_to_forecast():
	...
	ebt_margin_mu = IS['EBT Margin'][-lookbackForGrowth:].mean()
	ebt_margin_sigma = IS['EBT Margin'][-lookbackForGrowth:].std()
	...
	for s in range(num_simulations):
		...
		for i in range(1,periodsToPredict+1):
			...
			ebt_margin_used = np.random.normal(ebt_margin_mu, ebt_margin_sigma)
			...
			ebit_ = IS_forecast.loc['Operating Income'][f'Year{i}']
			tax_ = last_year_rev * ebt_margin_used * average_tax_rate
